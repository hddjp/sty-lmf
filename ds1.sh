...
for JOB_NAME in "${JOB_NAMES[@]}"; do
  rjob submit --gpu=8 --memory=1000000 -P 1 --cpu=128 --priority 8 --name="$JOB_NAME" \
  --host-network=true \
  ...
  -- bash -c "
    ...
    python -m llamafactory.cli train /mnt/shared-storage-gpfs2/ai4scifm-gpfs02/wuyixin/code/sty-lmf/train_scripts/${JOB_NAME}.yaml
    sudo chmod -R 777 /mnt/shared-storage-gpfs2/ai4scifm-gpfs02/wuyixin/code/sty-lmf/saves
    rm -r /mnt/shared-storage-gpfs2/ai4scifm-gpfs02/wuyixin/code/sty-lmf/saves/qwen3-4b/${JOB_NAME}/checkpoint-*/global_step*
  "
  sleep 3
done